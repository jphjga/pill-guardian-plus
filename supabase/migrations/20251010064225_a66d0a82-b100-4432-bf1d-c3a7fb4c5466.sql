-- ============================================
-- OPTION 1: Proper Role-Based Access Control
-- Fixed to handle existing data
-- ============================================
...
-- Step 2: Create role enum with all possible values
CREATE TYPE public.app_role AS ENUM ('administrator', 'manager', 'pharmacist', 'technician');
...
-- Step 6: Create function to get user's organization
...
-- Step 8: Update profiles policies - remove the problematic admin policy
DROP POLICY IF EXISTS "Administrators can update user roles" ON public.profiles;

-- Step 9: Add organization column to tables that don't have it
...
-- Step 10: Create trigger to auto-populate organization on inserts
...
FOR EACH ROW
EXECUTE FUNCTION public.set_organization_from_user();
...
-- Step 11: Add organization-based RLS to all data tables
